...
async def logo_gen(event):
    xx = await event.reply("`Creating logo......`")
    name = event.pattern_match.group(1)
    if not name:
        await xx.edit("`Provid Some txt 😏!\nExample: /logo <Anie-op>!`")
        return
    bg_, font_ = "", ""
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo by [Anie](https://t.me/Anierobot_bot)",
...
async def logo_(event):
    xx = await event.reply("`Creating wlogo wait....`")
    name = event.pattern_match.group(1)
    if not name:
